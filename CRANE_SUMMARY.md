...
```
Avg conf_text  = 0.9767   模型极度信任文本
Avg conf_audio = 0.3696   音频被选择性使用
Corr(conf_text, conf_audio) = -0.2022  互补学习：文本强时音频退让
```
...
**结果**：α=0.10 时 q≈1.46，coverage 90.7%，达到理论目标。
...
**结果**：α=0.10 时 coverage 91.0%（超理论值 1.0pp），中位宽度 2.92。**中位宽度小于平均宽度**——宽度分布右偏，确认为不确定样本分配了更宽的区间。
...
**结果**：α=0.10 时 overall coverage 90.5%，negative 88.7%、neutral 100.0%、positive 92.1%。Mondrian substantially improves conditional coverage across sentiment groups, with all groups achieving coverage close to the nominal target.
...
**结果**：α=0.10 时 coverage 90.2%，但中位宽度 4.19（比 MC Dropout 的 2.92 宽约 43%）。

**为什么更保守**：MVE yields more conservative intervals after conformal calibration compared to MC Dropout. 可能的原因为学习到的方差估计在校准集和测试集间的分布差异大于 MC Dropout 方差，但此解释需要进一步验证。

**价值**：证明了即便学习到的方差特性不同，conformal calibration 仍然能恢复有效覆盖。同时作为"learned uncertainty"的 ablation。
...
**结果**：α=0.10 时 coverage 88.3%，平均集合大小 2.93。Singleton rate 在所有 α≤0.15 时均为 0%——模型**做不到**以 85% 置信度确定一个唯一的情感类别，始终需要 2-3 个候选。
...
```
n_cal=20 : Coverage≈87.3%
n_cal=40 : Coverage≈91.3%  ← 40 个样本附近即稳定
n_cal=80+: Coverage≈91.0%  ← 之后持续稳定
```
...
```
α=0.10: Coverage=88.3%  Avg set size=2.93
...
*图9 — UBG 置信度与 Conformal 区间宽度的相关性：左面板展示文本置信度与自适应区间宽度的散点图及线性拟合（r≈0.13），按情感极性着色。右面板展示音频置信度与宽度的对应关系（r≈−0.04）。两个相关系数均接近零，直接排除了"UBG 在隐式学习预测不确定性"的可能性。这一负结果是 CRANE 架构合理性的核心证据——它证明 UBG 学到的是 modality reliability（"哪个模态更可信"）而非 predictive uncertainty（"预测有多不确定"），两者是不同维度。配合 conf_text=0.98、conf_audio=0.37、corr=−0.20 这些正结果，完整展示了 CRANE 的三层分工架构：UBG → 模态选择，MC Dropout → 不确定性估计，Conformal → 覆盖保证。三者职责清晰分离、互不替代——这是本工作除 conformal benchmark 之外最有区分度的贡献。*
...
To our knowledge, this is one of the first systematic studies of conformal uncertainty quantification for multimodal sentiment analysis on CMU-MOSI — covering Split, Adaptive, Mondrian, Classification Conformal, MVE, and Deep Ensemble in a unified comparison. 此前该数据集上的工作只报告点预测精度（MAE、Corr、Acc_7），没有一个模型能回答**"这个预测有多可靠"**。CRANE 提供了严格的不确定性保证——预测区间以 ≥90% 的概率包含真实情感分数，这**不是经验估计，而是数学上可证明的**。

### 3.2 条件覆盖保证揭示模型偏见

Mondrian Conformal 使每个情感类别的覆盖都接近标称目标。我们同时发现，当按预测值区间（而非真实情感极性）分组时，弱情感区域（[-1,0), [0,1)）的覆盖系统性下降到 85-88%——覆盖率在按情感极性分组时保持稳定，但在按预测值分桶时会在信号模糊区域下降。这两种分组方式揭示的是不同维度的模型行为。在此之前，这种偏见是不可见、不可测量的。
...
UBG 通过端到端训练**自动发现**了每个模态在情感分析中的价值（conf_text=0.98, conf_audio=0.37, corr=−0.20）。更重要的是，2.7 节的分析直接证明 UBG 学到的是 modality reliability 而非 predictive uncertainty——UBG 负责模态选择而非不确定性估计。这一"模块 A 和模块 B 没有学到同一个东西"的证据，在系统设计中具有方法论意义。
...
```
1. 情感分析需要不确定性量化
   → MC Dropout RAW 只有 34.4% coverage（差 56pp），Gaussian 假设完全错误

2. Conformal prediction 提供严格保证
   → 4 种基于区间的 conformal 方法全部达到或超过标称覆盖目标（≥90%）

3. CRANE 架构的三个差异化
   → UBG：自主学习模态互补（conf_text=0.98, conf_audio=0.37）
   → 双头输出：支持 MVE，覆盖达到 90.2%
   → Conformal Pipeline：模型无关，可复用

4. 重要发现
   → 在当前设置下 MC Dropout 产生比 4-member Deep Ensemble 更窄的区间
   → 在 CMU-MOSI 上约 40 个校准样本时覆盖率趋于稳定
   → UBG 负责模态选择、MC Dropout 负责不确定性、Conformal 负责覆盖——三者职责清晰分离
   → 弱情感区域是模型的系统性难点
