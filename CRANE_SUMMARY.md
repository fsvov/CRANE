...
```
Avg conf_text  = 0.9770   模型极度信任文本
Avg conf_audio = 0.4487   音频被选择性使用
Corr(conf_text, conf_audio) = -0.3485  互补学习：文本强时音频退让
```
...
|:---|:---:|:---:|
| Test MAE | 0.705 | **0.676** |
| Test Corr | 0.836 | **0.834** |
| Mult_acc_7 | 0.467 | 0.468 |
...
**结果**：α=0.10 时 q≈1.37，coverage 90.1%，达到理论目标。
...
**结果**：α=0.10 时 coverage 92.7%（超理论值 2.7pp），中位宽度 2.94。**中位宽度小于平均宽度**——宽度分布右偏，确认为不确定样本分配了更宽的区间。
...
**结果**：α=0.10 时 overall coverage 91.3%，negative 91.6%、neutral 100.0%、positive 89.9%。Negative 在本次运行中首次达到 90% 以上，Mondrian 的逐类保证全面生效。
...
**结果**：α=0.10 时 coverage 95.0%，但中位宽度 4.48（比 MC Dropout 的 2.94 宽 52%）。
...
**结果**：α=0.10 时 coverage 87.5%，平均集合大小 2.74。Singleton rate 在所有 α≤0.15 时均为 0%——模型**做不到**以 85% 置信度确定一个唯一的情感类别，始终需要 2-3 个候选。
...
|:---|:---:|:---:|:---:|
| MC Dropout RAW（Gaussian 假设） | **38.8%** ✗ | 0.74 | 0 |
| Split Conformal | **90.1%** ✓ | 2.73 | 0 |
| **Adaptive (MC Dropout)** | **92.7%** ✓ | 2.94 | 0 |
| Mondrian Conformal | **91.3%** ✓ | 3.22 | 0 |
| MVE + Adaptive | **95.0%** ✓ | 4.48 | 微调 |
| Deep Ensemble + Adaptive | 91.3% ✓ | 3.68 | **4×** |

**MC Dropout RAW 的灾难性失败**（38.8%）是第一个关键发现——证明无条件地信任不确定性估计是危险的。

**Adaptive (MC Dropout) 是最优方法**：零额外训练成本，覆盖超过理论保证，中位宽度 2.94。

**Deep Ensemble 不如 MC Dropout**：4 倍训练成本，区间反而更宽（3.68 vs 2.94），证明昂贵的集成学习在不确定性效率上劣于廉价的 MC Dropout。
...
```
n_cal=20 : Coverage=88.3%
n_cal=40 : Coverage=90.1%  ← 40 个样本就稳定
n_cal=80+: Coverage=90.1%  ← 之后持续稳定
```
...
```
α=0.10: Coverage=87.5%  Avg set size=2.74
...
UBG 通过端到端训练**自动发现**了每个模态在情感分析中的价值，并以显著的负相关（-0.35）实现互补融合，无需任何人工先验或特征工程。
...
```
1. 情感分析需要不确定性量化
   → MC Dropout RAW 只有 38.8% coverage（差 51pp），Gaussian 假设完全错误

2. Conformal prediction 提供严格保证
   → 5 种 conformal 方法全部达到或超过理论目标（≥90%）

3. CRANE 架构的三个差异化
   → UBG：自主学习模态互补（conf_text=0.98, conf_audio=0.45）
   → 双头输出：支持 MVE，覆盖提升到 95.0%
   → Conformal Pipeline：模型无关，可复用

4. 重要发现
   → MC Dropout 效率优于 Deep Ensemble（更窄 + 零训练成本）
   → 40 个校准样本足够
   → Audio 贡献 38% 的不确定性但只贡献少量信息增益
   → 弱情感区域是模型的系统性难点
