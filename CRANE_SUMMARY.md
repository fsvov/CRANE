...
**Adaptive (MC Dropout) 是零额外训练成本下的推荐方案**：覆盖超过理论保证，中位宽度 2.92。
...
纯音频区间是文本的约 1.9 倍。CRANE 的多模态融合在保持高覆盖的同时实现了较窄的区间，UBG 的负相关置信度分配是关键。
...
*图9 — UBG 置信度与 Conformal 区间宽度的相关性：左面板展示文本置信度与自适应区间宽度的散点图及线性拟合（r≈0.13），按情感极性着色（粉红=负面，米黄=中性，浅绿=正面）。右面板展示音频置信度与宽度的对应关系（r≈−0.04）。两个相关系数均接近零，表明 UBG 学到的模态置信度与 conformal 预测区间宽度之间不存在显著的线性相关性。这一发现澄清了 UBG 的功能定位：它学到的是"应该信任哪个模态"（模态间互补权重），而非"这个样本有多难预测"（预测难度估计）——两者是不同维度的不确定性。UBG 的负相关门控行为确保多模态融合优于单模态基线，但不确定性量化仍需要独立的 conformal calibration 层来提供覆盖保证。*
