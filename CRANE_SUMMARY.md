...
**原理**：SAGE-Net 在推理时保持 dropout 开启（K=20 次前向传播），用预测值的标准差 σ_mc 作为不确定性估计。然后假设误差服从正态分布，直接用 z 分数构造区间。
...
**结果**：α=0.10 时 q≈1.48，coverage 90.8%，达到理论目标。Constant width 3.0。
...
**结果**：α=0.10 时 coverage 92.7%（超理论值 2.7pp），中位宽度 2.97。注意**中位宽度小于平均宽度**——宽度分布右偏，证明确实对不确定样本分配了更宽的区间。

**为什么是最优**：(1) 零额外训练成本，仅需推理时 K=20 次前向传播；(2) 自适应——对确定样本窄、对不确定样本宽；(3) 中位宽度 2.97 在所有达标方法中最窄。
