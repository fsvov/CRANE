...
```
Avg conf_text  = 0.9548   模型极度信任文本
Avg conf_audio = 0.2648   音频被选择性使用
Corr(conf_text, conf_audio) = -0.1968  互补学习：文本强时音频退让
```
...
*图3 — UBG 学习到的模态置信度：双面板展示测试集上 UBG 为每个样本产生的置信度分数。左：文本置信度 vs. 音频置信度散点图，按情感极性着色（红色=负面，橙色=中性，绿色=正面）。左上角标注了 Pearson 相关系数——负值表明 UBG 学会了对模态进行互补门控（文本置信度高时，音频置信度倾向于降低，反之亦然）。数据点集中在高文本/低音频区域（x > 0.9，y ∈ [0.1, 0.5]），确认模型学到重度信任文本、选择性引入音频的策略。右：置信度边际分布的并列直方图。文本置信度（青色，μ≈0.95）在 1.0 附近尖锐集中，音频置信度（金色，μ≈0.26）在较低置信度范围内广泛分布，说明音频是按样本情况被选择性使用的，而非被统一忽略。*

![UBG Confidence vs Width](figures/fig9_ubg_confidence_vs_width.png)

*图9 — UBG 置信度与 Conformal 区间宽度的相关性分析：两个面板分别展示文本置信度（左）和音频置信度（右）与自适应 conformal 区间宽度的散点图及线性拟合。Corr(conf_text, width) ≈ 0.09，Corr(conf_audio, width) ≈ −0.03——两者均接近零，表明 UBG 门控置信度与 conformal 区间宽度之间不存在显著的线性相关性。这一发现说明 UBG 学到的"模态互补权重"与 conformal prediction 捕捉的"预测难度"是两个相对独立的信息维度：UBG 决定**相信哪个模态**，conformal 回答**预测有多不确定**。二者正交意味着将 UBG 置信度与 conformal 宽度结合可能提供更丰富的不确定性剖面。*
...
|:---|:---:|:---:|
| Test MAE | 0.705 | **0.667** |
| Test Corr | 0.836 | **0.836** |
| Mult_acc_7 | 0.467 | 0.461 |
...
**结果**：α=0.10 时 q≈1.51，coverage 92.4%，达到理论目标。
...
**结果**：α=0.10 时 coverage 93.2%（超理论值 3.2pp），中位宽度 2.83。**中位宽度小于平均宽度**——宽度分布右偏，确认为不确定样本分配了更宽的区间。

**为什么是推荐方案**：(1) 零额外训练成本，仅需推理时 K=20 次前向传播；(2) 自适应——对确定样本窄、对不确定样本宽；(3) 在所有零额外训练成本的达标方法中实现了最佳的覆盖-效率权衡。
...
**结果**：α=0.10 时 overall coverage 92.7%，negative 92.1%、neutral 100.0%、positive 92.8%。Mondrian substantially improves conditional coverage across sentiment groups, with all groups achieving coverage close to the nominal target.
...
**结果**：α=0.10 时 coverage 94.5%，但中位宽度 3.56（比 MC Dropout 的 2.83 宽约 26%）。
...
**结果**：α=0.10 时 coverage 89.9%，平均集合大小 3.01。Singleton rate 在所有 α≤0.15 时均为 0%——模型**做不到**以 85% 置信度确定一个唯一的情感类别，始终需要 3 个或更多候选。
...
|:---|:---:|:---:|:---:|
| MC Dropout RAW（Gaussian 假设） | **39.2%** ✗ | 0.70 | 0 |
| Split Conformal | **92.4%** ✓ | 3.01 | 0 |
| **Adaptive (MC Dropout)** | **93.2%** ✓ | 2.83 | 0 |
| Mondrian Conformal | **92.7%** ✓ | 3.39 | 0 |
| MVE + Adaptive | **94.5%** ✓ | 3.56 | 微调 |
| Deep Ensemble + Adaptive | 91.3% ✓ | 3.68 | **4×** |

**MC Dropout RAW 的灾难性失败**（39.2%）是第一个关键发现——证明无条件地信任不确定性估计是危险的。

**Adaptive (MC Dropout) 是零额外训练成本下的推荐方案**：覆盖超过理论保证，中位宽度 2.83。

**Deep Ensemble 的实证发现**：在我们的实验设置下（4-m embed ensemble, CMU-MOSI），Deep Ensemble 产生了更宽的 conformal 区间（3.68 vs 2.83），但此发现仅适用于当前配置，不代表 MC Dropout 在所有场景下优于 Deep Ensemble。
...
```
n_cal=20 : Coverage≈89.5%
n_cal=40 : Coverage≈92.3%  ← 40 个样本附近即稳定
n_cal=80+: Coverage≈93.2%  ← 之后持续稳定
```
...
纯音频区间是文本的约 1.8 倍。CRANE 的多模态融合在保持高覆盖的同时实现了较窄的区间，UBG 的负相关置信度分配是关键。
...
```
α=0.10: Coverage=89.9%  Avg set size=3.01
...
To our knowledge, this is among the first conformal uncertainty quantification frameworks for multimodal sentiment analysis on CMU-MOSI. 此前该数据集上的工作只报告点预测精度（MAE、Corr、Acc_7），没有一个模型能回答**"这个预测有多可靠"**。CRANE 提供了严格的不确定性保证——预测区间以 ≥90% 的概率包含真实情感分数，这**不是经验估计，而是数学上可证明的**。
...
UBG 通过端到端训练**自动发现**了每个模态在情感分析中的价值，并以负相关关系实现互补融合，无需任何人工先验或特征工程。
