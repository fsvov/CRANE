...
class MetricsTop():
    ...
    def map_to_class_sims(self, val):
        if val < -0.3:
            return -1
        elif val > 0.3:
            return 1
    ...
    def __eval_mosi_regression(self, y_pred, y_true, exclude_zero=False):
        ...
        #评估每一个类别的分类情况
        #y_pred_discrete = np.array([self.map_to_class(x) for x in test_preds])
        #y_true_discrete = np.array([self.map_to_class(x) for x in test_truth])
        #report = classification_report(y_true_discrete, y_pred_discrete , labels=[-3, -2, -1, 0, 1, 2, 3], output_dict=True)
       #for cls in [-3, -2, -1, 0, 1, 2, 3]:
        #    cls_report = report.get(str(cls)) if isinstance(list(report.keys())[0], str) else report.get(cls)
        #    if cls_report:
        #        eval_results[f"class_{cls}_precision"] = round(cls_report["precision"], 4)
        #        eval_results[f"class_{cls}_f1"] = round(cls_report["f1-score"], 4)
        #        eval_results[f"class_{cls}_support"] = int(cls_report["support"])
        return eval_results
...
class ConformalMetrics:
    @staticmethod
    def format_results(y_true, y_pred, lower, upper, alpha, label=""):
        ...
        cov = compute_coverage(yt, lo, up)
        avg_w, med_w = compute_interval_width(lo, up)
        score = compute_interval_score(yt, lo, up, alpha)
        tag = f" [{label}]" if label else ""
        return (
            f"α={alpha:.2f}{tag}: "
            f"Coverage={cov:.4f}  "
            f"Avg_Width={avg_w:.4f}  "
            f"Med_Width={med_w:.4f}  "
            f"Interval_Score={score:.4f}"
        )

    @staticmethod
    def format_conditional_sentiment(y_true, y_pred, lower, upper):
        from utils.conformal import conditional_coverage_by_sentiment
        results = conditional_coverage_by_sentiment(y_true, y_pred, lower, upper)
        lines = []
        for label, info in results.items():
            lines.append(
                f"  {label:<10}  Count={info['count']:>4}  "
                f"Coverage={info['coverage']:.4f}  "
                f"Avg_Width={info['avg_width']:.4f}"
            )
        return "\n".join(lines)

    @staticmethod
    def format_conditional_bucket(y_pred, y_true, lower, upper):
        from utils.conformal import conditional_coverage_by_bucket
        results = conditional_coverage_by_bucket(y_pred, y_true, lower, upper)
        lines = []
        for label, info in results.items():
            lines.append(
                f"  {label:<10}  Count={info['count']:>4}  "
